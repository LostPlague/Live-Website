...
//
// The LOGIC lives here, outside functions/, on purpose. Netlify refuses HTTP
// access to scheduled functions (an empty-body 403), so a test trigger bolted
// onto the scheduled function itself is unreachable — which would leave the
// alerting unverifiable, and unverified alerting is the kind you only discover
// is broken by the alert that never came. So: this module holds the work,
// functions/alerts.mjs runs it on the cron, and functions/alerts-run.mjs
// exposes the same thing over HTTP behind the admin password.
//
// Zero dependencies: it queries PostHog with the key already in the env and
// hands the email to FormSubmit, the same service the contact form already
// uses for this address.
//
...
/**
 * Do the actual work. Shared by the cron and the HTTP trigger.
 * @param {{dryRun?: boolean, test?: boolean}} opts
 *   dryRun — report what WOULD be sent, mail nothing.
 *   test   — send one obviously-labelled email to prove delivery works.
 * @returns {Promise<object>} a plain result object, safe to serialise.
 */
export async function runAlerts(opts = {}) {
  if (!process.env.POSTHOG_PERSONAL_KEY) {
    throw new Error('server not configured — set POSTHOG_PERSONAL_KEY in Netlify');
  }
  ...
  if (!rows.length) return { ok: true, found: 0 };
  ...
  return { ok: true, found: rows.length, emailed: true };
}
